    ...
    document.getElementById('clean-invalid-btn')?.addEventListener('click', () => {
        ...
        if (btn) {
            ...
            setTimeout(() => {
                ...
                btn.innerText = '🧹 Vyčistiť galériu';
            }, 3000);
        }
    });
...
// Shared weak-metadata check for both HornySimp and NSFW247
function hasWeakDetailMetadata(video) {
    if (!video) return false;
    if (!isHornySimpVideo(video) && !isNsfw247Video(video)) return false;
...
async function startHornySimpMetadataResolution() {
    ...
    try {
        ...
        for (let i = 0; i < targets.length; i += BATCH_SIZE) {
            ...
            const urls = chunk.map(v => String(v.source_url || v.url || '').trim()).filter(Boolean);
            const batch = await fetchHornySimpMetadataBatchInTab(activeTab.id, urls);
